...
有两种合理路线：API-first（后端优先）或 UI-first（前端优先）。个人开发者通常 UI-first（快速看到东西）。但这个项目数据模型简单（评论点赞），UI-first 更符合博客项目的实际。
...
1. **设计先行**：先写设计文档（本项目就是personal-blog-design.md），确定“做什么、用什么技术、分几期”
...
# 1：工程骨架（能 npm install）

## 1.1 根目录三个文件

- **package.json**：声明 `workspaces: ["apps/*"]`（单仓双应用），定义顶层脚本 `dev`（同时起 API 和 Web）、`db:push` 等——你以后只在这一个目录跑命令
...
- **apps/web/package.json**：依赖 astro、vue、@astrojs/vue 集成；脚本 `dev`/`build`/`typecheck`
- **apps/web/astro.config.ts**：注册 Vue 集成、配置 `site`（RSS/sitemap 依赖它）、**Vite 代理 `/api` → `localhost:8787`**——这是前后端同域的关键，前端请求 `/api/xxx` 开发时自动转发给 API
- **apps/web/tsconfig.json**：TypeScript 编译规则
- **src/env.d.ts**：让 TS 认识 `.astro`、`.vue` 文件的类型声明

## 1.3 apps/api 骨架（3 个文件）

- **apps/api/package.json**：依赖 hono、drizzle-orm、postgres（驱动）、@node-rs/argon2（密码哈希）、zod（校验）
- **apps/api/tsconfig.json**：API 侧编译规则
- **apps/api/drizzle.config.ts**：告诉 drizzle-kit“方言是 postgresql、schema 在哪、数据库连哪”——`npm run db:push` 就靠它

---

# 阶段 2：内容模型（博客本质是内容站，先定义“文章是什么”）

## 2.1 apps/web/src/content.config.ts
...
## 2.2 content/posts/*.md（3篇示例文章）

每篇带完整 frontmatter。有了它们，接下来写的每个页面都能**立刻看到真实效果**。

## 2.3 apps/web/src/lib/posts.ts
...
## 2.4 apps/web/src/styles/global.css

设计 token（CSS 变量）：深色主题配色、蓝紫渐变强调色、明暗两套变量。先写样式地基，后面所有组件只引用变量，视觉天然统一。

---

# 阶段 3：公开页面（访客看的部分，静态输出）

**顺序原则：先布局 → 再卡片类复用组件 → 最后按路由逐页写。**

## 3.1 布局与通用组件（4 个文件）

- **apps/web/src/layouts/BaseLayout.astro**：全站外壳——`<html>` 骨架、SEO meta、导航栏、页脚、主题初始化脚本（防闪烁）。所有页面都包它
- **apps/web/src/components/ThemeToggle.astro**：明暗切换，纯原生 JS 小脚本（不值得上 Vue）
- **apps/web/src/components/PostCard.astro**：文章卡片（标题+摘要+标签+日期），首页/列表/标签页复用
- **apps/web/src/components/PostToc.astro**：文章目录，消费 `extractHeadings()` 的输出

## 3.2 公开路由（设计文档第 3 节的页面清单）

Astro 的约定：`src/pages/` 下的文件路径即路由。

- **apps/web/src/pages/index.astro**：首页——个人介绍+精选+最新文章
- **apps/web/src/pages/posts/index.astro**：文章列表
- **apps/web/src/pages/posts/[slug].astro**：文章详情——渲染 Markdown、目录、阅读进度条，并**预埋**点赞/评论两个 Vue 岛的位置（此时组件还不存在，先用占位）
- **apps/web/src/pages/about.astro**：关于页
- **apps/web/src/pages/tags/[tag].astro**：标签筛选页
- **apps/web/src/pages/search.astro** + **apps/web/src/pages/search.json.ts**：后者构建期生成全站搜索索引（标题+摘要的 JSON），前者加载它做客户端过滤——静态站也能有搜索，不需要搜索引擎服务
- **apps/web/src/pages/rss.xml.js**：RSS 端点 + sitemap 集成，SEO 三件套收尾
...
- **apps/api/src/db/schema.ts**：**整个后端最重要的文件**。用 Drizzle 定义 6 张表——`profiles`（用户）、`sessions`（会话）、`post_drafts`（草稿）、`comments`、`likes`（含 `(user_id, content_id)` 唯一约束，天然防重复点赞）、`sync_jobs`。表结构直接翻译设计文档第 5 节
- **apps/api/src/db/index.ts**：创建 postgres 连接 + Drizzle 实例，全项目唯一的数据库入口
- **apps/api/src/db/seed.ts**：幂等种子——首次启动按 `.env` 创建管理员，已存在就跳过

写完立刻 `npm run db:push` 验证：schema 能推上数据库，等于数据模型成立。

## 4.3 基础设施（4 个文件）

- **apps/api/src/types.ts**：跨模块共享的 TS 类型（如返回给前端的用户信息）
- **apps/api/src/lib/validation.ts**：zod schema 复用（邮箱/密码/评论正文的格式规则），路由层调它
- **apps/api/src/middleware/rateLimit.ts**：滑动窗口限流，防止登录/评论接口被刷（设计文档第 7 节）
- **apps/api/src/services/session.ts**：会话管理——生成随机令牌 → 哈希后落库 → 写 HttpOnly Cookie。独立成 service 因为登录/登出/鉴权三处都要用

## 4.4 apps/api/src/middleware/auth.ts

两个 Hono 中间件：`requireUser`（没登录就 401）和 `requireAdmin`（非管理员就 403）。路由统一挂它，权限校验只写一次——这是“路由层校验权限”的实现。

---

## 5：认证与互动路由（API 长出第一批端点）

- **apps/api/src/routes/auth.ts**：注册（Argon2id 哈希密码）、登录（防时序攻击的校验）、登出、`/me`（当前用户）。**第一个写的路由**，因为它最独立、能用 curl 直接验证，成功后立刻确认“env→db→middleware→service”整条链路通了
- **apps/api/src/routes/posts.ts**：按 `contentId` 的点赞（toggle，唯一约束保证幂等——重复请求返回当前状态而非报错）和评论（创建默认 `pending`，公开读取只返回 `approved`）
- **apps/api/src/app.ts**：把所有路由挂到 Hono 实例 + **统一错误出口**（捕获异常、区分错误类型、绝不把堆栈/密钥返回客户端）
- **apps/api/src/index.ts**：入口——启动 HTTP 服务、跑种子、启动同步 worker（下阶段填）
...
- **apps/web/src/components/vue/LoginForm.vue**：登录+注册表单，成功后跳转。配 **apps/web/src/pages/login.astro**（页面只做布局，交互全在岛里）
- **apps/web/src/components/vue/LikeButton.vue**：点赞按钮——未登录引导去登录，已登录 toggle，乐观更新
- **apps/web/src/components/vue/CommentSection.vue**：评论区——加载已审核评论 + 提交新评论（提示“审核中”，失败保留输入，设计文档第 9 节）
- **apps/web/src/components/vue/AccountPanel.vue**：账户页的昵称修改、登出。配 **apps/web/src/pages/account.astro**

回到 `[slug].astro` 把占位换成真正的 `<LikeButton client:visible />`、`<CommentSection client:visible />`——**`client:visible` 表示滚动到可见才水合**，文章页上半屏保持零 JS。另外 **apps/web/src/components/vue/SearchResults.vue** 驱动搜索页交互。

---

# 阶段 7：发布链路与后台（最复杂，最后做）

**为什么最后做**：它依赖前面所有东西（草稿表、会话、权限、GitHub 配置），且自身链路最长。

## 7.1 发布链路（API 侧 3 个文件）

- **apps/api/src/services/github.ts**：GitHub Contents API 的薄封装——生成 frontmatter + Markdown、创建/更新文件
- **apps/api/src/services/sync.ts**：**同步处理器，核心逻辑**——领取任务（状态 pending→processing）、调 GitHub、成功记 commit SHA + 触发草稿转 published；失败按**指数退避**算下次重试时间（10s→20s→40s…），超过最大次数转 failed 等手动重试。幂等性靠 `(draftId, revision)` 唯一
- **apps/api/src/workers/syncWorker.ts**：`setInterval` 轮询到期任务交给 sync 处理。**设计取舍**：不用独立进程，直接嵌在 API 里——个人项目少一个部署单元，量级完全够

## 7.2 后台路由（routes/admin/，全挂 requireAdmin）

- **apps/api/src/routes/admin/drafts.ts**：草稿 CRUD + **发布**（保存 DB → 创建 sync_job，事务保证草稿不丢）
- **apps/api/src/routes/admin/comments.ts**：评论列表（含 pending）/通过/隐藏/删除
- **apps/api/src/routes/admin/sync.ts**：同步任务列表 + 手动重试
- **apps/api/src/routes/admin/index.ts**：后台路由聚合导出

## 7.3 后台页面与组件（Web 侧最后一波）

- **apps/web/src/pages/admin/index.astro**：后台首页，聚合三个管理面板
- **apps/web/src/pages/admin/posts/index.astro** + **apps/web/src/components/vue/AdminPostList.vue**：草稿列表——显示状态（草稿/待同步/已发布/同步失败）、发布、删除
- **apps/web/src/pages/admin/posts/edit.astro** + **apps/web/src/components/vue/AdminEditor.vue**：编辑器——标题/摘要/标签/slug/封面/精选的表单 + Markdown 正文编辑 + 实时预览 + 保存/发布。静态站下用 `?id=` 传参由组件拉取草稿
- **apps/web/src/components/vue/AdminComments.vue**：审核台——pending 通过、已发布隐藏
- **apps/web/src/components/vue/AdminSyncJobs.vue**：同步任务监控——每次尝试、错误信息、手动重试按钮
